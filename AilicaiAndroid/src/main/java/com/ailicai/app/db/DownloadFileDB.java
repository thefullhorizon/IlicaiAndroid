...
/**
 * 数据库中文件下载记录的操作，包括：插入，删除，更新
 *
 * @author shenyamin
 */
public class DownloadFileDB {
    // 数据库字段 path, thid, done
     ...
     * 插入数据
     *
     * @param model DownLoadFileModel实体类
     */
...
//					+ " WHERE " + COL_PATH + "=? AND " + COL_THID + " = -9999";// -9999已经下载完成的标识
        ...
        try {
            ...
            if (c.moveToNext()) {
                ...
                if (result == len) {
                    ...
                    // 删除下载记录后，插入一条新的记录包含本次下载的url,apk的大小，-9999作为标识以便下次提取这条记录
