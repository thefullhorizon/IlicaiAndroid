...
package com.ailicai.app.widget.mpchart.renderer;
...
import com.ailicai.app.widget.mpchart.utils.ColorTemplate;
import com.ailicai.app.widget.mpchart.utils.FSize;
            ...
            case HORIZONTAL: {
                ...
                for (int i = 0, count = labels.length; i < count; i++) {
                    ...
                    if (drawingForm) {
                        ...
                        if (mLegend.getForms().size() >= count) {
                            mLegend.setForm(mLegend.getForms().get(i));
                        }
    ...
    protected void drawForm(Canvas c, float x, float y, int index, Legend legend) {
        ...
        switch (legend.getForm()) {
            case CIRCLE:
                ...
                if (legend.getHoleRadius() !=0) {
                    //Added by wulianghuan 2016-05-03 绘制圆环中的白色小圆圈
                    Paint circlePaintInner = new Paint(Paint.ANTI_ALIAS_FLAG);
                    circlePaintInner.setStyle(Paint.Style.FILL);
                    circlePaintInner.setColor(Color.WHITE);
                    c.drawCircle(x + half, y, legend.getHoleRadius(), circlePaintInner);
                }
                break;
            case SQUARE:
